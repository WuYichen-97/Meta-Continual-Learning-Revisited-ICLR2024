...
class MNISTMLP_MAML(MammothBackbone):  # __init__.py
    """
    ...
    def __init__(self, input_size: int, output_size: int) -> None:
        ...
        """
        super(MNISTMLP_MAML, self).__init__()
    ...
    def functional_forward(self,x:torch.Tensor,fast_weight:OrderedDict,returnt:str ='out')-> torch.Tensor:
        ...
        x = x.view(-1, num_flat_features(x))
        x = F.relu(F.linear(x,fast_weight['fc1.weight'], fast_weight['fc1.bias']))
        feats = F.relu(F.linear(x,fast_weight['fc2.weight'], fast_weight['fc2.bias']))
        if returnt == 'features':
            return feats
