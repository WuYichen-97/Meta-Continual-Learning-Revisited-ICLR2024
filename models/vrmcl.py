...
class VRMCL(ContinualModel):
    ...
    def take_multitask_loss(self, bt, logits, y, task_id):
        ...
        for i, ti in enumerate(bt):
            if i < 32:  # 32 is the adopted batchsize 
    ...
    def outer_loop(self):           
        ...
        if self.args.asyn_update: 
            ...
            with torch.no_grad():
                # vr-mcl
                for g, p in zip(self.net.ema.parameters(), self.net.main.parameters()):
    ...
    def observe(self, inputs: torch.Tensor, labels: torch.Tensor, not_aug_inputs, task_id):
        ...
        if self.observed_batch <= self.max_batch_in_minitask:
            self.buffer.add_data(examples=not_aug_inputs, logits =None,  labels=labels, task_labels=task_id*torch.ones_like(labels))
