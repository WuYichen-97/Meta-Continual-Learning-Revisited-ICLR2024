...
class Buffer:
    ...
    def __init__(self, buffer_size, device, n_tasks=None, mode='reservoir'):
        ...
        # for class balance resovior sampling
        self.largest = []
        self.each_class_dic = {}
    ...
    def most_common_labels(self, labels):
        counter = Counter(labels.cpu().tolist())
        max_count = max(counter.values())
        # print('max_count',max_count)
        largest = [label for label, count in counter.items() if  count ==max_count ]
        return largest
    ...
    def add_data(self, examples, labels=None, logits=None, task_labels=None):
        ...
        # N_c number of stream instances of class c
        for label in labels:
            if label.cpu().tolist() in self.each_class_dic:
                self.each_class_dic[label.cpu().tolist()] += 1
            else:
                self.each_class_dic[label.cpu().tolist()] = 1
            # print('dict0',label, self.each_class_dic)

        for i in range(examples.shape[0]):
            # determin the largest classes in the buffer
            if self.num_seen_examples > self.buffer_size:
                for label in self.most_common_labels(self.labels):
                    # print('label',label, self.largest)
                    if label not in self.largest:
                        self.largest.append(label)
                # print('self.largest',self.largest)
                # max_labels = self.most_common_labels(self.labels)
                # print('max_labels',max_labels)
                if labels[i].cpu().tolist() not in self.largest :
                    max_labels = self.most_common_labels(self.labels)
                    # print('max_labels',max_labels)
                    index = random.choice([j for j,label in enumerate(self.labels) if label in max_labels])
                    ...
                    m_c = len([j for j,label in enumerate(self.labels) if label == labels[i]])
                    N_c = self.each_class_dic[labels[i].cpu().tolist()]
                    # print('mc,Nc',m_c,N_c)
                    if random.random()<= m_c/N_c:
                        index = random.choice([j for j,label in enumerate(self.labels) if label ==labels[i]])
                        self.examples[index] = examples[i].to(self.device)
                        if labels is not None:
                            self.labels[index] = labels[i].to(self.device)
                        if logits is not None:
